...
# ------------------------------------------- FUNCTIONS -----------------------------------------------
    ...
    #TO COMMENT.
    def load_images(self, image_names, prep_function=True,color_mode=False):
        ...
        return load_multiple_images(self.src_dataset, image_names, color_mode, self.target_size,
                                    preprocessing_function=self.preprocessing_function,
                                    prep_function=prep_function)

    def draw_rf_on_images(self, img_name,crop_pos, K, P, S,mode='constant'):
        ...
        img2 = self._load_image(img_name)
        # Add padding

        img=np.pad(img, ((P, P), (P, P), (0, 0)), mode)
        img2 = np.pad(img2, ((P+1, P+1), (P+1, P+1), (0, 0)), mode)
        img = np.pad(img, ((1, 1), (1, 1), (0, 0)))
        img[crop_pos[0]*S:crop_pos[0]*S+K+2, crop_pos[1]*S:crop_pos[1]*S+K+2] = [255,0,0]
        img[crop_pos[0] * S +1:crop_pos[0] * S + K +1, crop_pos[1] * S+1:crop_pos[1] * S + K +1] = img2[crop_pos[0] * S +1:crop_pos[0] * S + K +1, crop_pos[1] * S+1:crop_pos[1] * S + K+1 ]

        return img

    def get_patch(self, img_name, crop_pos, K, P, S, mode='constant'):
        """Returns a region patch from an image.
        ...
        if P > 0:
            img = np.pad(img, ((P, P), (P, P), (0, 0)), mode)
            img2 = img[crop_pos[0] * S:crop_pos[0] * S + K, crop_pos[1] * S:crop_pos[1] * S + K,:]
        else:
            img2=img

        return img2

    def get_patch_out(self, img_name, crop_pos, K, P, S,model, mode='constant'):
        """Returns a region patch from an image.
        ...
        :return: PIL image instance.
        """
        img = self._load_image(img_name,self.preprocessing_function)
        img = model(*img).data.cpu()
    ...
    def get_patch_old(self, img_name, crop_pos=None):
        """Returns a region patch from an image.
